<template>
    ...
    <div class="nav-container">
        ...
        <div class="blog-container">
            <el-card v-for="(blog, index) in userBlogs" :key="index" class="blog-card">
                <template #header>
                    <h3>题目：{{ blog.title }}</h3>
                    <h4>作者：{{ otherUsername }}</h4>
...
const activeIndex = ref('otherProfile');
...
const fetchUserBlogs = async () => {
    try {
        const { username: otherUsernameFromParams } = router.currentRoute.value.params;
        otherUsername.value = otherUsernameFromParams;
        const response = await axios.get(`${apiUrl}/api/getUserBlogs/${otherUsernameFromParams}`);
        userBlogs.value = response.data.blogs;
    } catch (error) {
